...
if __name__ == "__main__":
    ...
    def _run_worker():
        try:
            ...
            logging.exception("document-edit-worker: worker thread crashed — health-check still running")

    t = threading.Thread(target=_run_worker, daemon=True)
    t.start()

    port = int(os.environ.get("PORT", "8080"))
    logging.info("document-edit-worker: serving health-check on port %s", port)
